...
export const sendSellerEmailOtp = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    await otpTable.upsert({ user_id: auth.user.id, email: data.email, code_hash: hash(data.email, code), attempts: 0, expires_at: new Date(Date.now() + 10 * 60_000).toISOString() }, { onConflict: "user_id,email" });
    const key = process.env.RESEND_API_KEY;
    if (!key) throw new Error("Email verification is not configured");
    const response = await fetch("https://api.resend.com/emails", { method: "POST", headers: { Authorization: `Bearer ${key}`, "Content-Type": "application/json" }, body: JSON.stringify({ from: process.env.SELLER_OTP_FROM ?? "Local Shore <onboarding@resend.dev>", to: [data.email], subject: "Local Shore seller verification code", text: `Your seller verification code is ${code}. It expires in 10 minutes.` }) });
...
export const verifySellerEmailOtp = createServerFn({ method: "POST" })
  .inputValidator((d: { accessToken: string; email: string; code: string }) => {
    if (!d?.accessToken || !emailOk(d.email) || !/^\d{6}$/.test(d.code)) throw new Error("Invalid verification request");
  ...
  .handler(async ({ data }) => {
    ...
    if (authError || !auth.user || auth.user.email?.toLowerCase() !== data.email) throw new Error("Session/email mismatch");
    const otpTable = supabaseAdmin.from("seller_verification_otps" as any) as any;
    const { data: row } = await otpTable.select("id,code_hash,attempts,expires_at").eq("user_id", auth.user.id).eq("email", data.email).maybeSingle();
    if (!row || new Date(row.expires_at).getTime() < Date.now() || row.attempts >= 5) throw new Error("Code expired or locked");
