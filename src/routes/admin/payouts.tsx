...
export const Route = createFileRoute("/admin/payouts")({
  head: () => ({
    meta: [{ title: "Vendor Payouts — Admin" }, { name: "robots", content: "noindex" }],
  }),
...
function PayoutsPage() {
  ...
  const exportCsv = () => {
    const rows = q.data ?? [];
    const csv = [
      "settlement_id,seller_id,gross_sales,commission,gst_on_fees,net_payout,status,created_at",
      ...rows.map((r: any) =>
        [
          r.id,
          r.seller_id,
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-2">
        <div>
          <h1 className="text-2xl font-black tracking-tight sm:text-3xl">Vendor Payouts</h1>
          <p className="text-sm text-muted-foreground">
            Release, hold and reconcile vendor settlements.
          </p>
        </div>
                ...
                >
                  <div className="min-w-0">
                    <div className="truncate font-medium">
                      Settlement #{String(s.id).slice(0, 8)}
                    </div>
                    <div className="text-xs text-muted-foreground">
                      Gross {fmt(s.gross_sales)} · Fees{" "}
                      {fmt(Number(s.commission || 0) + Number(s.gst_on_fees || 0))} · Net{" "}
                      {fmt(s.net_payout)}
                    </div>
                  </div>
