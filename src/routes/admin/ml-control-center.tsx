...
export const Route = createFileRoute("/admin/ml-control-center")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Tune machine learning ranking weights and monitor search intelligence.",
      },
    ],
  }),
...
export function MLControlCenterPage() {
  ...
  const saveWeights = useMutation({
    mutationFn: async (newWeights: ModelWeights) => {
      const { error } = await (supabase as any).from("ml_model_configs").upsert(
        {
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Tune multi-factor shop ranking algorithms, evaluate search accuracy, and inspect local
            discovery metrics.
          </p>
        </div>
        ...
        <Card>
          <CardContent className="py-4">
            ...
            <div className="text-2xl font-black text-indigo-400 mt-1">
              {analytics.conversion_rate || 22.4}%
            </div>
        ...
        <Card className="border-indigo-500/20">
          <CardHeader>
            ...
            <CardDescription className="text-xs">
              Adjust the weight of each component in the shop discovery score matrix. Total must
              equal 100%.
            </CardDescription>
          </CardHeader>
            ...
            {[
              {
                ...
                desc: "Exponential proximity score boost (< 2 km)",
                color: "bg-emerald-500",
              },
              {
                key: "w_stock",
                label: "In-Stock Quantity Weight",
                desc: "Available stock ratio score",
                color: "bg-blue-500",
              },
              {
                key: "w_availability",
                label: "Open Store Status Weight",
                desc: "Open now vs closed schedule score",
                color: "bg-indigo-500",
              },
              {
                key: "w_rating",
                label: "Rating & Trust Weight",
                desc: "Customer review rating score",
                color: "bg-amber-500",
              },
              {
                key: "w_delivery_speed",
                label: "Delivery Speed Weight",
                desc: "Predicted prep + transit time score",
                color: "bg-purple-500",
              },
              {
                key: "w_velocity",
                label: "Historical Velocity Weight",
                desc: "Recent order & click popularity score",
                color: "bg-pink-500",
              },
            ].map((item) => {
            ...
            <div className="space-y-2 pt-2">
              ...
              {simResults.length === 0 ? (
                ...
                simResults.map((item, idx) => (
                  <div
                    key={idx}
                    className="p-3 rounded-lg bg-card/80 border border-border/60 flex flex-col gap-1.5"
                  >
                    <div className="flex items-center justify-between">
