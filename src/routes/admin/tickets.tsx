...
export const Route = createFileRoute("/admin/tickets")({
  head: () => ({
    meta: [{ title: "Support Tickets — Admin" }, { name: "robots", content: "noindex" }],
  }),
...
function TicketsPage() {
  ...
  return (
    <div className="space-y-4">
      <div>
        <h1 className="text-2xl font-black tracking-tight sm:text-3xl">Support Tickets</h1>
        <p className="text-sm text-muted-foreground">Customer and vendor support conversations.</p>
      </div>
      <div className="flex flex-wrap items-center gap-2">
        ...
        <span className="mr-1 text-xs font-medium text-muted-foreground">Stage</span>
        {[
          "all",
          "submitted",
          "under_review",
      ...
      {!q.isLoading && (
        <p className="text-xs text-muted-foreground">
          Showing {rows.length} of {(q.data ?? []).length} support request(s).
        </p>
      )}
      {q.isLoading ? (
            ...
            <Card key={t.id}>
              <CardContent className="p-4 space-y-2">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <Badge
                    variant={
                      t.support_stage === "rejected" || t.support_stage === "refunded"
                        ? "secondary"
                        : "default"
                    }
                    className="capitalize ml-auto"
                  >
                    {(t.support_stage ?? t.status).replaceAll("_", " ")}
                  </Badge>
                </div>
                <p className="text-sm text-muted-foreground">{t.body}</p>
                {t.issue_type && (
                  <div className="flex flex-wrap gap-3 text-xs text-muted-foreground">
                    <span>Issue: {t.issue_type.replaceAll("_", " ")}</span>
                    <span>{t.selected_product_ids?.length ?? 0} item(s)</span>
                    <span>
                      {(t.evidence_urls?.length ?? 0) + (t.video_url ? 1 : 0)} evidence file(s)
                    </span>
                    {t.order_id && <span>Order: {t.order_id.slice(0, 8)}</span>}
                  </div>
                )}
                <div className="flex flex-wrap gap-2">
                  <Button
                    ...
                    onClick={() =>
                      void transition(
                        t.id,
                        t.support_stage ?? "submitted",
                        "under_review",
                        "Case opened for review",
                      )
                    }
                  >
                    Review
                  ...
                  <Button
                    ...
                    onClick={() =>
                      void transition(
                        ...
                        t.support_stage ?? "submitted",
                        "awaiting_shop_response",
                        "Shop response requested",
                      )
                    }
                  >
                    Request shop response
                  </Button>
                  <Button
                    size="sm"
                    variant="outline"
                  ...
                  <Button
                    ...
                    onClick={() =>
                      void transition(
                        ...
                        t.support_stage ?? "submitted",
                        "replacement_delivered",
                        "Replacement marked delivered",
                      )
                    }
                  >
                    Mark replacement delivered
                  </Button>
                  {((t.evidence_urls?.length ?? 0) > 0 || Boolean(t.video_url)) && (
                    <EvidenceLinks
                      paths={[...(t.evidence_urls ?? []), ...(t.video_url ? [t.video_url] : [])]}
                    />
                  )}
                </div>
              </CardContent>
  ...
  async function transition(id: string, from: string, to: string, note: string) {
    ...
    try {
      ...
      if (!updated)
        throw new Error("Support case was not updated. Check your admin access and try again.");
      const { data: auth } = await supabase.auth.getUser();
      if (auth.user) {
        const { error: eventError } = await (supabase as any)
          .from("support_case_events")
          .insert({ ticket_id: id, actor_id: auth.user.id, from_stage: from, to_stage: to, note });
...
function EvidenceLinks({ paths }: { paths: string[] }) {
  ...
  return (
    <Button size="sm" variant="outline" disabled={loading} onClick={() => void openEvidence()}>
