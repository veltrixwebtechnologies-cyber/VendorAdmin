...
function InventoryPage() {
  ...
  const rows = useMemo(() => onlyLow ? products.filter((p) => p.stock <= p.lowStockAt) : products, [products, onlyLow]);
  ...
  function draftValue(id: string, current: number) { return drafts[id] ?? current; }
  ...
  async function commit(id: string, current: number) {
    ...
    try {
      ...
      setDrafts((d) => { const { [id]: _dropped, ...rest } = d; void _dropped; return rest; });
      toast.success("Stock updated");
    } catch (e: any) { toast.error(e?.message ?? "Failed"); }
  }
  async function setLowAt(id: string, v: number) {
    try { await updateStock.mutateAsync({ id, lowStockAt: v }); }
    catch (e: any) { toast.error(e?.message ?? "Failed"); }
  }
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Update stock levels and configure low-stock alerts.</p>
      </div>

      <div className="grid gap-3 sm:grid-cols-3">
        <StatCard label="Total units" value={summary.totalUnits} icon={<Box className="h-4 w-4" />} />
        <StatCard label="Low stock" value={summary.low} icon={<AlertTriangle className="h-4 w-4 text-accent-foreground" />} />
        <StatCard label="Out of stock" value={summary.outOfStock} tone="destructive" icon={<AlertTriangle className="h-4 w-4" />} />
      </div>
                      ...
                      <TableRow key={p.id} className="animate-fade-in">
                        ...
                        <TableCell>
                          <div className="flex items-center gap-1">
                            <Button size="icon" variant="outline" className="h-8 w-8" onClick={() => bump(p.id, p.stock, -1)}><Minus className="h-3.5 w-3.5" /></Button>
                            <Input type="number" min={0} value={current}
                              onChange={(e) => setDrafts((d) => ({ ...d, [p.id]: Math.max(0, Number(e.target.value) || 0) }))}
                              className="h-8 w-20 text-center" />
                            <Button size="icon" variant="outline" className="h-8 w-8" onClick={() => bump(p.id, p.stock, 1)}><Plus className="h-3.5 w-3.5" /></Button>
                          </div>
                        </TableCell>
                        <TableCell>
                          <Input type="number" min={0} defaultValue={p.lowStockAt}
                            onBlur={(e) => { const v = Math.max(0, Number(e.target.value) || 0); if (v !== p.lowStockAt) setLowAt(p.id, v); }}
                            className="h-8 w-24" />
                        </TableCell>
                        <TableCell>
                          {out ? <Badge className="bg-destructive text-destructive-foreground">Out of stock</Badge>
                            : low ? <Badge className="bg-accent text-accent-foreground">Low</Badge>
                            : <Badge className="bg-success text-success-foreground">In stock</Badge>}
                        </TableCell>
                        <TableCell className="text-right">
                          <Button size="sm" variant={dirty ? "default" : "ghost"} disabled={!dirty || updateStock.isPending} onClick={() => commit(p.id, p.stock)}>
...
function StatCard({ label, value, icon, tone }: { label: string; value: number; icon: React.ReactNode; tone?: "destructive" }) {
  return (
    <Card><CardContent className="flex items-center justify-between py-4">
      ...
      <div className={"grid h-9 w-9 place-items-center rounded-lg " + (tone === "destructive" ? "bg-destructive/10 text-destructive" : "bg-primary/10 text-primary")}>
        {icon}
      </div>
    </CardContent></Card>
  );
