...
export const Route = createFileRoute("/seller/hours")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Set your shop opening hours, closures, and holiday schedule.",
      },
    ],
  }),
...
function LiveStatusChip({ sellerId }: { sellerId: string }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 rounded-full border px-3 py-1 text-sm font-semibold ${colorMap[status] ?? "bg-muted text-muted-foreground"}`}
    >
      <span
        className={`h-2 w-2 rounded-full ${isOpen ? "bg-emerald-500 animate-pulse" : "bg-red-500"}`}
      />
      {label}
    </span>
  );
...
function OverridePanel({ sellerId }: { sellerId: string }) {
  ...
  return (
    <div className="rounded-xl border border-border bg-card p-5 space-y-4">
      ...
      {active && (
        <div
          className={`rounded-lg border p-3 text-sm ${active.kind === "manual_open" ? "bg-blue-50 border-blue-200 text-blue-800" : "bg-amber-50 border-amber-200 text-amber-800"}`}
        >
          ...
          {active.effectiveUntil && (
            <p className="text-xs mt-0.5">
              Until: {new Date(active.effectiveUntil).toLocaleString()}
            </p>
          )}
        </div>
      )}
        ...
        <div className="space-y-3 rounded-lg bg-muted/50 p-4">
          <div>
            ...
            <input
              ...
              placeholder="e.g. Staff shortage, Renovations…"
              className="mt-1 w-full rounded-md border border-border bg-background px-3 py-1.5 text-sm outline-none focus:ring-2 focus:ring-primary/30"
            />
          </div>
          <div>
            <label className="text-xs font-medium text-muted-foreground">
              Close until (leave blank for indefinite)
            </label>
            <input
              type="datetime-local"
              value={until}
    ...
    <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 py-3 border-b border-border last:border-0">
      <div className="flex items-center gap-3 w-28 shrink-0">
        <Switch
          ...
          id={`dow-${hour.dayOfWeek}`}
        />
        <label htmlFor={`dow-${hour.dayOfWeek}`} className="text-sm font-medium w-8">
          {DAY_FULL[hour.dayOfWeek].slice(0, 3)}
        </label>
      </div>
      {hour.isOpen ? (
        <div className="flex flex-wrap items-center gap-2 flex-1">
          <input
            ...
            className="rounded-md border border-border bg-background px-2.5 py-1.5 text-sm outline-none focus:ring-2 focus:ring-primary/30 w-[120px]"
          />
      ...
      {showForm && (
        <form onSubmit={handleAdd} className="space-y-3 rounded-lg bg-muted/50 p-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div className="flex items-center gap-2 pt-5">
              <Switch
                ...
                id="h-is-closed"
              />
              <label htmlFor="h-is-closed" className="text-sm">
                {form.isClosed ? "Full Day Closure" : "Special Hours"}
              </label>
            </div>
          </div>
          ...
          {!form.isClosed && (
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-xs font-medium text-muted-foreground">Special Open</label>
                <input
                  type="time"
                  value={form.specialOpen}
                  onChange={(e) => setForm((f) => ({ ...f, specialOpen: e.target.value }))}
                  className="mt-1 w-full rounded-md border border-border bg-background px-3 py-1.5 text-sm outline-none focus:ring-2 focus:ring-primary/30"
                />
              </div>
              <div>
                <label className="text-xs font-medium text-muted-foreground">Special Close</label>
                <input
                  type="time"
                  value={form.specialClose}
                  onChange={(e) => setForm((f) => ({ ...f, specialClose: e.target.value }))}
                  className="mt-1 w-full rounded-md border border-border bg-background px-3 py-1.5 text-sm outline-none focus:ring-2 focus:ring-primary/30"
                />
              </div>
            </div>
            ...
            >
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  ...
                  {h.isClosed
                    ? "Closed"
                    : `${fmt12(h.specialOpen ?? "")} – ${fmt12(h.specialClose ?? "")}`}
                </p>
              </div>
              <Button
                size="icon"
                ...
                onClick={() => delHoliday.mutate({ id: h.id, sellerId })}
              >
...
function ShopHoursPage() {
  ...
  if (
    seller &&
    tz === "Asia/Kolkata" &&
    (seller as any).timezone &&
    (seller as any).timezone !== tz
  ) {
  ...
  function updateDay(dow: number, patch: Partial<ShopHour>) {
    setLocalHours((prev) =>
      (prev ?? hours).map((h) => (h.dayOfWeek === dow ? { ...h, ...patch } : h)),
    );
  }
  ...
  async function handleSave() {
    ...
    if (hasError) {
      toast.error("Fix time errors before saving");
      return;
    }
    try {
  ...
  if (!seller || seller.status !== "approved") {
    return (
      <div className="rounded-xl border border-dashed border-muted p-8 text-center text-sm text-muted-foreground">
        Your shop must be approved before you can configure hours.
      </div>
    );
  }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">
            Set when your shop is open. Override or add special events any time.
          </p>
        </div>
