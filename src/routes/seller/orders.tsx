    ...
    <div className="mx-auto max-w-6xl space-y-6">
        ...
        <TabsList className="flex flex-wrap h-auto">
          {TABS.map((t) => (
            <TabsTrigger key={t.value} value={t.value} className="gap-2">
              {t.label}<Badge variant="secondary" className="ml-1">{counts[t.value] ?? 0}</Badge>
            </TabsTrigger>
          ))}
        </TabsList>
      </Tabs>
      ...
      {q.isLoading ? (
        ...
        <div className="grid gap-3">
          {filtered.map((o) => (
            <button key={o.id} onClick={() => setOpenId(o.id)} className="animate-fade-in text-left">
          ...
          <>
            ...
            <div className="mt-4 space-y-5">
              <section className="rounded-lg border border-border p-3">
                <div className="mb-2 text-xs font-semibold uppercase text-muted-foreground">Buyer</div>
                <div className="font-medium">{order.buyerName}</div>
                <div className="mt-1 flex items-center gap-2 text-sm text-muted-foreground"><Phone className="h-3.5 w-3.5" /> {order.buyerPhone}</div>
                <div className="mt-1 flex items-start gap-2 text-sm text-muted-foreground">
                  <MapPin className="mt-0.5 h-3.5 w-3.5 shrink-0" />
                  <span>{order.buyerAddress}{order.city ? `, ${order.city}, ${order.state}` : ""}{order.pincode ? ` — ${order.pincode}` : ""}</span>
                </div>
              </section>
              ...
              <section>
                ...
                <div className="space-y-2">
                  {order.items.map((it) => (
                    <div key={it.id} className="flex justify-between rounded-md border border-border p-2 text-sm">
                      <div>
                        <div className="font-medium">{it.name}</div>
                        <div className="text-xs text-muted-foreground">SKU: {it.sku}</div>
                      </div>
                      <div className="text-right">
                        <div>₹{it.price.toLocaleString("en-IN")} × {it.qty}</div>
                        <div className="font-medium">₹{(it.price * it.qty).toLocaleString("en-IN")}</div>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <div className="space-y-1 text-sm">
                  ...
                  <div className="flex justify-between"><span className="text-muted-foreground">Shipping</span><span>₹{order.shipping.toLocaleString("en-IN")}</span></div>
                  <div className="flex justify-between font-semibold"><span>Total ({order.paymentMode})</span><span>₹{order.total.toLocaleString("en-IN")}</span></div>
                </div>
              </section>

              {order.awb && (
                <section className="rounded-lg border border-border p-3">
                  <div className="mb-1 flex items-center gap-2 text-xs font-semibold uppercase text-muted-foreground"><Truck className="h-3.5 w-3.5" /> Shipping</div>
            ...
            <AlertDialog open={cancelOpen} onOpenChange={setCancelOpen}>
              <AlertDialogContent>
                <AlertDialogHeader>
                  ...
                  <AlertDialogDescription>The buyer will be notified. Cancellations may impact your seller rating.</AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
                  <AlertDialogCancel>Keep order</AlertDialogCancel>
                  <AlertDialogAction onClick={async () => {
                    try { await cancel.mutateAsync({ id: order.id, reason: "Cancelled by seller" }); toast.success("Order cancelled"); setCancelOpen(false); }
                    catch (e: any) { toast.error(e?.message ?? "Failed"); }
