...
export const Route = createFileRoute("/seller/analytics")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Sales trend, top products, fulfilment funnel and seller performance.",
      },
...
function AnalyticsPage() {
  ...
  const perf = useMemo(() => {
    ...
    const listingScore = products.length
      ? (products.filter((p) => p.status === "active").length / products.length) * 100
      : 0;
    ...
    <div className="mx-auto max-w-6xl space-y-6 animate-fade-in">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Last 14 days across your catalog and orders.
        </p>
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Card>
          ...
          <CardContent className="h-64">
            {catMix.length === 0 ? (
              <EmptyChart label="Add products to see categories" />
            ) : (
              <ResponsiveContainer width="100%" height="100%">
        ...
        <Card>
          ...
          <CardContent>
            <div className="flex items-end justify-between gap-6">
              ...
              <div className="grid gap-2 text-sm">
                <PerfRow
                  ...
                  good={kpi.cancelRate < 5}
                />
                <PerfRow
                  label="Listing quality"
                  value={perf.listingScore + "%"}
                  good={perf.listingScore >= 80}
                />
                <PerfRow
                  label="Delivered orders"
                  value={String(perf.onTime)}
                  good={perf.onTime > 0}
                />
              </div>
            </div>
            <p className="mt-4 text-xs text-muted-foreground">
              ...
              Combines fulfilment, cancellation and catalog health.
            </p>
          </CardContent>
        </Card>
      </div>
