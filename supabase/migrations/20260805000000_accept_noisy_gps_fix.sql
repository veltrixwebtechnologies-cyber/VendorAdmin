-- Browsers can return a delayed/coarse fix that makes the apparent speed
-- impossible. Do not reject the entire location update: keep the coordinate,
-- omit the unreliable speed value, and allow dispatch to use the fresh fix.
...
BEGIN
  ...
  IF p.status <> 'approved' THEN RAISE EXCEPTION 'Only approved delivery partners can update live location'; END IF;
  IF p.availability NOT IN ('online', 'busy') THEN RAISE EXCEPTION 'Go online before sharing live location'; END IF;
