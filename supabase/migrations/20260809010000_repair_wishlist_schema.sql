-- Ensure the customer wishlist tables and policies exist in shared
-- production projects, even when an older migration batch was skipped.
...
ON public.wishlist_entries FOR ALL TO authenticated
USING (user_id = auth.uid())
WITH CHECK (user_id = auth.uid());
